...
/*Here I declared the password length variable. The logic applied to restrict character length to be between 8 - 128 can be found in a while loop within
the prompt() function. I placed a placehold interger of 0, but the full assignment logic for this variable is located with the while loop.*/
...
The generatePassword() and buildCriteria() below shows why I created this empty string variable.*/
var master = "";

/*This logic outlined in this buildCriteria function is used to append the password criteria variables into the master password variable based on the users answers
to the prompting questions listed in the prompts() function. The new variables listed in this function will allow the prompting questions and user selection answers to pass
through the logic of this function.
...
/*The generatePassword() function will generate a secure password that follows the criteria selected by the user. This function includes a for loop and establishes a 
new local password variable that will be used to return the generated secure password that will define the securePassword variable listed at the end of the prompt() function.*/
...
/*The prompts() function groups all password criteria questionts into a single function, which will be triggered once the user clicks the "Generate Password" button 
in the index.html file. The buildCriteria() function is invoked within the prompts() function to append any user selected criteria to the empty master variable intialized in
...
function prompts() {
	...
	/**This while loop with require the user to enter how long they would like for their secure password to be. Since the passwordLength variable was intialized
	with a value of 0, the isNan() will ensure users  recieve an initial prompt to enter a password length after the special character question. If the user does 
	not enter a number of characters within the range of allowed number of characters, they will continue to recieve the prompt below.*/
	...
	/*NOTE - The while loop for the passwordLength variable above ensures the user is prompted to specify how long they would like their secure password to be, so
	a numerical value of total desired characters will be defined when this prompts() function is invoked.*/
	...
	/*Resetting the passwordLength variable to 0 to ensure that the password length resets after the prompts() function finished running and/or the user want to generate a new 
	password and re-clicks the "Generate Password" button. I did this because I noticed some edge cases where the password length would not reset unless the web page
